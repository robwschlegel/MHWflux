...
rmse_wrap <- function(df_sub){
  qnet_lag <- c(0, df_sub$qnet_mld_cum[1:nrow(df_sub)-1]) + df_sub$sst[1]
  lhf_lag <- c(0, df_sub$lhf_mld_cum[1:nrow(df_sub)-1]) + df_sub$sst[1]
  shf_lag <- c(0, df_sub$shf_mld_cum[1:nrow(df_sub)-1]) + df_sub$sst[1]
  lwr_lag <- c(0, df_sub$lwr_mld_cum[1:nrow(df_sub)-1]) + df_sub$sst[1]
  swr_lag <- c(0, df_sub$swr_mld_cum[1:nrow(df_sub)-1]) + df_sub$sst[1]
  df_res <- df_sub %>% 
    summarise(qnet_mld_cum = rmse(sst, qnet_lag),
              lhf_mld_cum = rmse(sst, lhf_lag),
              shf_mld_cum = rmse(sst, shf_lag),
              lwr_mld_cum = rmse(sst, lwr_lag),
              swr_mld_cum = rmse(sst, swr_lag)) %>% 
