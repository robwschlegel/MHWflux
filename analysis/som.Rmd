...
for(i in 1:nrow(OISST_MHW_event)){
  ...
  event_overlap <- OISST_MHW_event %>% 
    dplyr::rename(region_match = region, 
                  event_no_match = event_no,
                  duration_match = duration) %>% 
    filter(date_start %in% seq(df$date_start, df$date_end, by = "day"),
           date_end %in% seq(df$date_start, df$date_end, by = "day")) %>%
    ...
    # Remove events that have already been counted
    mutate(strng3 = paste(region, event_no, region_match, event_no_match),
           strng4 = paste(region_match, event_no_match, region, event_no)) %>% 
    filter(!strng4 %in% event_overlap_res$strng3)

  # Combine
  event_overlap_res <- rbind(event_overlap, event_overlap_res)
...
event_overlap_tidy <- plyr::ddply(OISST_MHW_event, c("region", "event_no"), overlap_check, .parallel = T) %>% 
  # Remove duplicates
