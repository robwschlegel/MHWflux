...
ALL_RMSE_top_wide <- rbind(ALL_RMSE_top_region, ALL_RMSE_top_season) %>% 
  pivot_wider(names_from = Parameter2, values_from = count) %>% 
  mutate(Qlw = replace_na(Qlw, 0))  # Never most important for one MAB onset row
  # pivot_wider(names_from = c(Parameter2, ts), values_from = count) %>% 
  # mutate(Qlw_onset = replace_na(Qlw_onset, 0))  # Never most important for MAB onset
knitr::kable(ALL_RMSE_top_wide)
```
...
```{r RMSE-stats}
# Short event index
short_event_index <- ALL_RMSE %>% 
  filter(n_Obs <= 10, ts == "full") %>% 
  dplyr::select(region, season, event_no, n_Obs) %>% 
  unique()

# The top Qx for events of 10 days or less
ALL_RMSE_short_top_region <- ALL_RMSE %>%
  right_join(short_event_index, by = c("region", "season", "event_no", "n_Obs")) %>% 
  group_by(region, ts, Parameter2) %>%
  filter(rmse == min(rmse)) %>% 
  summarise(count = n(), .groups = "drop") %>% 
  dplyr::rename(group = region)
ALL_RMSE_short_top_season <- ALL_RMSE %>% 
  right_join(short_event_index, by = c("region", "season", "event_no", "n_Obs")) %>% 
  group_by(season, ts, Parameter2) %>% 
  summarise(count = n(), .groups = "drop") %>% 
  dplyr::rename(group = season)
ALL_RMSE_short_top_wide <- rbind(ALL_RMSE_short_top_region, ALL_RMSE_short_top_season) %>% 
  pivot_wider(names_from = Parameter2, values_from = count)
knitr::kable(ALL_RMSE_short_top_wide)
```
