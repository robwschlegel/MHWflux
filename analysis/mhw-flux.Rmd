...
events_cor_prep <- GLORYS_MHW_event %>% 
  ...
  ungroup() %>% 
  dplyr::select(region:n_Obs, temp, bottomT, sss, mld_cum, mld_1_cum, t2m, tcc_cum, p_e_cum, msl_cum,
...
events_cor_prep %>% 
  ...
  pivot_longer(cols = temp:qnet_mld_cum) %>% 
  filter(name != "temp",
         ts != "full") %>% 
...
events_cor_prep %>% 
  ungroup() %>% 
  dplyr::select(rate_onset, ts, temp, bottomT, sss, mld_cum, mld_1_cum, t2m, tcc_cum, p_e_cum, msl_cum,
                 msnlwrf_mld_cum, msnswrf_mld_cum, mslhf_mld_cum, msshf_mld_cum, qnet_mld_cum) %>% 
