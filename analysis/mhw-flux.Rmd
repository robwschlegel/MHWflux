...
ALL_mag <- readRDS("data/ALL_cor.Rda") %>% 
  dplyr::select(region:ts, Parameter1, Parameter2, n_Obs, mag) %>% 
  na.omit()
...
ALL_mag_prop <- ALL_mag %>% 
  ...
  left_join(filter(ALL_mag, Parameter2 == "sst"), 
            by = c("region", "season", "event_no", "ts", "Parameter1", "n_Obs")) %>% 
  filter(ts != "full") %>% 
  mutate(prop = mag.x/mag.y)

# Find an event with good onset and decline match
ALL_mag_prop %>% 
  filter(Parameter2.x == "qnet_budget",
         prop >= 1, prop <= 2)
...
# Table showing the count of best magnitude match per variable per region/season
```
