...
```{r linearity}
# The events and their RMSE scores
ALL_cor_R2 <- readRDS("data/ALL_cor.Rda") %>% 
  ungroup() %>% 
  filter(Parameter1 == "sst",
         rmse > 0) %>% 
  dplyr::select(region:ts, Parameter2, r, n_Obs, rmse)

# The SSTa data
SST_R2 <- ALL_anom %>%
  filter(var == "sst") %>% 
  dplyr::select(region, t, anom) %>% 
  left_join(OISST_MHW_clim[,c("region", "t", "event_no")], by = c("region", "t")) %>% 
  filter(event_no > 0) %>% 
  # group_by(region, event_no) %>% 
  nest_by(region, event_no)
  # mutate(event_index = 1:n())
  # mutate_if(is.numeric, cumsum)

# Function that calculates R2 and compares to RMSE
