...
These figures are helpful, but we also want a table showing the count of best magnitude match per variable per region/season.

```{r mag-table}
# Get total 
mag_region_count <- ALL_mag_prop %>% 
  dplyr::select(region:ts, n_Obs) %>% 
  unique() %>% 
  group_by(region, ts) %>% 
  summarise(count = n(), .groups = "drop")
mag_season_count <- ALL_mag_prop %>% 
  dplyr::select(region:ts, n_Obs) %>% 
  unique() %>% 
  group_by(season, ts) %>% 
  summarise(count = n(), .groups = "drop")
...
These tables help to reiterate how much more important Qswr is for the onset and decline of MHWs over the other variables, and that Qlhf often comes in second. For the seasons though we see a much more complex relationship that I won't go into here.
...
knitr::kable(ALL_RMSE_region_season)
```
