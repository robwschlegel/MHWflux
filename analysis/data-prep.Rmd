...
saveRDS(OISST_grid[,1:2], "metadata/OISST_grid.Rds")
...
GLORYS_grid <- GLORYS_grid_base %>% 
  mutate(OISST_index = as.vector(knnx.index(as.matrix(OISST_grid[,c("lon", "lat")]),
                                            as.matrix(GLORYS_grid[,c("lon", "lat")]), k = 1))) %>%
...
saveRDS(GLORYS_grid, "metadata/GLORYS_grid.Rds")
...
# Combine for visual
ALL_regions <- rbind(OISST_regions, GLORYS_regions) %>%
...
system.time(
GLORYS_all_ts <- plyr::ldply(GLORYS_files_hires[1:5], load_GLORYS, .parallel = T, region = T) %>%
  dplyr::arrange(region, t) %>% 
  mutate(cur_spd = round(sqrt(u^2 + v^2), 4),
         cur_dir = round((270-(atan2(v, u)*(180/pi)))%%360))
) # 284 seconds on 10 cores
...
# Load and prep the GLORYS low-res data
system.time(
GLORYS_all <- plyr::ldply(GLORYS_files, load_GLORYS, .parallel = T) %>% 
  dplyr::rename(mld = mlp) %>% 
  mutate(lon = lon+0.125, lat = lat-0.125,
         mld = round(mld, 4), u = round(u, 6), v = round(v, 6)) %>% 
  right_join(OISST_grid, by = c("lon", "lat")) %>% 
  dplyr::select(lon, lat, t, u, v, mld) %>% 
  na.omit()
) # 159 seconds on 25 cores

# Calculates clims+anoms and save
system.time(
GLORYS_all_anom <- GLORYS_all %>% 
  pivot_longer(cols = c(-lon, -lat, -t), names_to = "var", values_to = "val") %>% 
  plyr::ddply(., c("lon", "lat", "var"), calc_clim_anom, .parallel = T, point_accuracy = 6)
) # xxx seconds on 25 cores
