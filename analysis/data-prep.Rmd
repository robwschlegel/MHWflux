...
box_comp <- MHW_event %>% 
  ...
  labs(fill = "Region", colour = "Product", x = NULL, y = "Value fo given facet",
...
ALL_anom <- ALL_ts %>%
  ...
  mutate(qnet_mld = qnet/(mld*1042*4000),
         msnlwrf_mld = msnlwrf/(mld*1042*4000),
         msnswrf_mld = msnswrf/(mld*1042*4000),
         mslhf_mld = mslhf/(mld*1042*4000),
         msshf_mld = msshf/(mld*1042*4000),
         mld_1 = 1/mld) %>% 
...
ALL_anom_cum <- ALL_anom %>%
  ...
  dplyr::select(region:tcc, msl, qnet, p_e, mld, mld_1, qnet_mld:msshf_mld) %>% 
...
# This is no longer necessary as the Qx/MLD calcs are now done in the first anomaly calcs above 
