...
ALL_anom <- ALL_ts %>%
  ...
  pivot_longer(msnlwrf:cur_spd, names_to = "var", values_to = "val") %>% 
  group_by(region, var) %>%
  nest() %>%
...
ALL_anom_cum <- ALL_anom %>%
  ...
  dplyr::select(region:msshf, msl, qnet, tcc, p_e) %>% 
  ...
  pivot_longer(cols = msnlwrf:p_e, names_to = "var", values_to = "anom") %>% 
