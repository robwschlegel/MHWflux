...
We also need to create cummulative heatflux terms. This is done by taking the first day during the MHW and adding them to each other as we go along. This daily cumm product should then be divided by the MLD on that day. This aproximates the air-sea flux term. We also need water density and secific heat constant. Q/(rho x Cp x hmld). rho = 1042, Cp ~= 4000. And then these daily air-sea flux values are added together each day to create the cumm flux time series. This is done for Qnet, not the individual terms

The movement terms arent very useful and may not be worth including as they dont really show advection. So rather one can say that the parts of the heating that arent exlained by anything else could be attributed to advection through the process of elimination.
